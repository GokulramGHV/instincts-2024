...
export async function generateMetadata({ params }) {
  const title = params['event-name'];

  return {
    title: `Instincts 2024 | ${title.toUpperCase()}`,
    description: `Instincts 2024 | ${title.toUpperCase()}`,
  };
}
  ...
  return (
    <main className="px-5 sm:px-10 py-5">
      <div className="flex items-center pb-3">
        ...
        <p
          className={`${londonFont.className} text-5xl sm:text-[80px] text-[#202020] sm:ml-5`}
        >
          {eventDetails[params['event-name']]['name']}
        </p>
      </div>
      <div className="sm:px-16 px-6">
        <p className="text-justify font-satoshi sm:text-xl text-[#5C5C5C]">
          {eventDetails[params['event-name']]['description']}
        </p>
        <div className="grid  sm:grid-cols-3 gap-5 mt-5">
          {eventDetails[params['event-name']]['images'].map((eventImage) => {
            return (
              <div
                key={eventImage}
                className="w-full  bg-[#5C5C5C] col-span-1 rounded-lg overflow-hidden"
              >
                <Image
                  src={eventImage}
                  width={200}
